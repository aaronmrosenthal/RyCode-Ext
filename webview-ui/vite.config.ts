...
export default defineConfig(({ mode }) => {
	...
	const define: Record<string, any> = {
		...
		"process.env.PKG_OUTPUT_CHANNEL": JSON.stringify("RyCode-Ext"),
		...(gitSha ? { "process.env.PKG_SHA": JSON.stringify(gitSha) } : {}),
	}

	// TODO: We can use `@rycode-ext/build` to generate `define` once the
	...
	if (mode === "nightly") {
		...
		define["process.env.PKG_OUTPUT_CHANNEL"] = JSON.stringify("RyCode-Ext-Nightly")
	}
	...
	return {
		...
		resolve: {
			alias: {
				...
				"@rycode-ext": resolve(__dirname, "../src/shared"),
			},
		},
		build: {
			outDir,
