...
import { getGitSha, copyPaths, copyLocales, copyWasms, generatePackageJson } from "@rycode-ext/build"
...
async function main() {
	...
	const buildOptions = {
		...
		platform: "node",
		define: {
			"process.env.PKG_NAME": '"rycode-ext-nightly"',
			"process.env.PKG_VERSION": `"${overrideJson.version}"`,
			"process.env.PKG_OUTPUT_CHANNEL": '"RyCode-Ext-Nightly"',
			...
			setup(build) {
				build.onEnd(() => {
					...
					const generatedPackageJson = generatePackageJson({
						packageJson,
						overrideJson,
						substitution: ["roo-cline", "rycode-ext-nightly"],
					})
