...
export async function createRun({ suite, exercises = [], systemPrompt, timeout, ...values }: CreateRun) {
	...
	try {
		...
		const cliCommand = `pnpm --filter @rycode-ext/evals cli --runId ${run.id}`
		...
		const logStream = fs.createWriteStream("/tmp/rycode-ext-evals.log", { flags: "a" })
