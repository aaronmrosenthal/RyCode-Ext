...
import { type TokenUsage, RyCodeExtEventName, taskEventSchema } from "@rycode-ext/types"
import type { Run, Task, TaskMetrics } from "@rycode-ext/evals"
...
export const useRunStatus = (run: Run): RunStatus => {
	...
	const onMessage = useCallback((messageEvent: MessageEvent) => {
		...
		switch (eventName) {
			case RyCodeExtEventName.TaskStarted:
				startTimes.current.set(taskId, Date.now())
				break
			case RyCodeExtEventName.TaskTokenUsageUpdated: {
			...
			case RyCodeExtEventName.EvalPass:
			case RyCodeExtEventName.EvalFail:
