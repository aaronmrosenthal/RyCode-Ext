...
suite.skip("RyCode-Ext Subtasks", () => {
	test("Should handle subtask cancellation and resumption correctly", async () => {
		const api = globalThis.api

		const messages: Record<string, ClineMessage[]> = {}

		api.on(RyCodeExtEventName.Message, ({ taskId, message }) => {
		...
		api.on(RyCodeExtEventName.TaskSpawned, (_, childTaskId) => (spawnedTaskId = childTaskId))
