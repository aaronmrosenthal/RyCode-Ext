// npx vitest core/ignore/__tests__/RyCodeExtIgnoreController.spec.ts

import type { Mock } from "vitest"

import { RyCodeExtIgnoreController, LOCK_TEXT_SYMBOL } from "../RyCodeExtIgnoreController"
...
describe("RyCodeExtIgnoreController", () => {
	const TEST_CWD = "/test/path"
	let controller: RyCodeExtIgnoreController
	...
	beforeEach(() => {
		...
		// Create controller
		controller = new RyCodeExtIgnoreController(TEST_CWD)
	})

	describe("initialization", () => {
		/**
		 * Tests the controller initialization when .rycodeextignore exists
		 */
		it("should load .rycodeextignore patterns on initialization when file exists", async () => {
			// Setup mocks to simulate existing .rycodeextignore file
			...
			expect(mockFileExists).toHaveBeenCalledWith(path.join(TEST_CWD, ".rycodeextignore"))
			expect(mockReadFile).toHaveBeenCalledWith(path.join(TEST_CWD, ".rycodeextignore"), "utf8")
		...
		/**
		 * Tests the controller behavior when .rycodeextignore doesn't exist
		 */
		it("should allow all access when .rycodeextignore doesn't exist", async () => {
			// Setup mocks to simulate missing .rycodeextignore file
		 ...
		 */
		it("should set up file watcher for .rycodeextignore changes", async () => {
			// Check that watcher was created with correct pattern
			expect(vscode.workspace.createFileSystemWatcher).toHaveBeenCalledWith(
				expect.objectContaining({
					base: TEST_CWD,
					pattern: ".rycodeextignore",
				}),
			)
		 ...
		 */
		it("should handle errors when loading .rycodeextignore", async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith("Unexpected error loading .rycodeextignore:", expect.any(Error))
	...
	describe("validateAccess", () => {
		...
		/**
		 * Tests the default behavior when no .rycodeextignore exists
		 */
		it("should allow all access when no .rycodeextignore content", async () => {
			// Create a new controller with no .rycodeextignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new RyCodeExtIgnoreController(TEST_CWD)
	...
	describe("validateCommand", () => {
		...
		/**
		 * Tests behavior when no .rycodeextignore exists
		 */
		it("should allow all commands when no .rycodeextignore exists", async () => {
			// Create a new controller with no .rycodeextignore
			mockFileExists.mockResolvedValue(false)
			const emptyController = new RyCodeExtIgnoreController(TEST_CWD)
			await emptyController.initialize()
	...
	describe("getInstructions", () => {
		/**
		 * Tests instructions generation with .rycodeextignore
		 */
		it("should generate formatted instructions when .rycodeextignore exists", async () => {
			// Setup .rycodeextignore content
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules\n.git\nsecrets/**")
			await controller.initialize()
		 ...
		 * Tests behavior when no .rycodeextignore exists
		 */
		it("should return undefined when no .rycodeextignore exists", async () => {
			// Setup no .rycodeextignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
	...
	describe("file watcher", () => {
		/**
		 * Tests behavior when .rycodeextignore is created
		 */
		it("should reload .rycodeextignore when file is created", async () => {
			// Setup initial state without .rycodeextignore
			mockFileExists.mockResolvedValue(false)
			await controller.initialize()
			...
			// Create and initialize controller with no .rycodeextignore
			controller = new RyCodeExtIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Force reload of .rycodeextignore content manually
			await controller.initialize()
		...
		/**
		 * Tests behavior when .rycodeextignore is changed
		 */
		it("should reload .rycodeextignore when file is changed", async () => {
			// Setup initial state with .rycodeextignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
		...
		/**
		 * Tests behavior when .rycodeextignore is deleted
		 */
		it("should reset when .rycodeextignore is deleted", async () => {
			// Setup initial state with .rycodeextignore
			mockFileExists.mockResolvedValue(true)
			mockReadFile.mockResolvedValue("node_modules")
			await controller.initialize()
