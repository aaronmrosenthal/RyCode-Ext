...
import type { ExperimentId } from "@rycode-ext/types"
import { DEFAULT_TERMINAL_OUTPUT_CHARACTER_LIMIT } from "@rycode-ext/types"
	...
	if (includeFileDetails) {
		...
		if (isDesktop) {
			...
			if (maxFiles === 0) {
				...
				const { showRyCodeExtIgnoredFiles = false } = state ?? {}
