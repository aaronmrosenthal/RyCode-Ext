// npx vitest core/prompts/__tests__/responses-rycodeextignore.spec.ts
...
describe("RyCodeExtIgnore Response Formatting", () => {
	...
	describe("formatResponse.rooIgnoreError", () => {
		...
		it("should format error message for ignored files", () => {
			...
			expect(errorMessage).toContain("Access to secrets/api-keys.json is blocked by the .rycodeextignore file settings")
			expect(errorMessage).toContain("continue in the task without using this file")
			expect(errorMessage).toContain("ask the user to update the .rycodeextignore file")
		})
	...
	describe("formatResponse.formatFilesList with RyCodeExtIgnoreController", () => {
		/**
		 * Tests file listing with rycodeextignore controller
		 */
		it("should format files list with lock symbols for ignored files", async () => {
			// Create controller
			const controller = new RyCodeExtIgnoreController(TEST_CWD)
		...
		/**
		 * Tests formatFilesList when showRyCodeExtIgnoredFiles is set to false
		 */
		it("should hide ignored files when showRyCodeExtIgnoredFiles is false", async () => {
			// Create controller
			const controller = new RyCodeExtIgnoreController(TEST_CWD)
			await controller.initialize()
			...
			// Format with controller and showRyCodeExtIgnoredFiles = false
			const result = formatResponse.formatFilesList(
				TEST_CWD,
				files,
				false,
				controller as any,
				false, // showRyCodeExtIgnoredFiles = false
			)
		...
		/**
		 * Tests formatFilesList handles truncation correctly with RyCodeExtIgnoreController
		 */
		it("should handle truncation with RyCodeExtIgnoreController", async () => {
			// Create controller
			const controller = new RyCodeExtIgnoreController(TEST_CWD)
			await controller.initialize()
		 ...
		 */
		it("should handle empty file list with RyCodeExtIgnoreController", async () => {
			// Create controller
			const controller = new RyCodeExtIgnoreController(TEST_CWD)
			await controller.initialize()
	...
	describe("getInstructions", () => {
		 ...
		 */
		it("should format .rycodeextignore instructions for the LLM", async () => {
			// Create controller
			const controller = new RyCodeExtIgnoreController(TEST_CWD)
			await controller.initialize()
		...
		it("should return undefined when no .rycodeextignore exists", async () => {
			// Set up no .rycodeextignore
			mockFileExists.mockResolvedValue(false)

			// Create controller without .rycodeextignore
			const controller = new RyCodeExtIgnoreController(TEST_CWD)
			await controller.initialize()
