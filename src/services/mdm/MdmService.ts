...
export class MdmService {
	...
	private getMdmConfigPath(): string {
		...
		switch (platform) {
			case "win32": {
				// Windows: %ProgramData%\RyCodeExt\mdm.json or mdm.dev.json
				const programData = process.env.PROGRAMDATA || "C:\\ProgramData"
				return path.join(programData, "RyCodeExt", configFileName)
			}

			case "darwin":
				// macOS: /Library/Application Support/RyCodeExt/mdm.json or mdm.dev.json
				return `/Library/Application Support/RyCodeExt/${configFileName}`

			case "linux":
			default:
				// Linux: /etc/rycode-ext/mdm.json or mdm.dev.json
				return `/etc/rycode-ext/${configFileName}`
		}
	}
